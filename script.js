...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const analyzeSpecialTeiaFormat = (rawResponsesArray) => {
        ...
        let responsesMeetingPartCriterion = 0;
        const aggregatedCounts = {}; // { Categoria: { Resposta: contagem } }
        const foundCategories = new Set();
        // Regex para extrair Resposta (grupo 1) e Categoria (grupo 2) de UMA parte JÁ SEGMENTADA
        const specialPartRegex = /^(.*?)\s+\(([^)]+)\)$/;
        const MIN_PART_PERCENTAGE = 0.5; // 50% das partes

        // Regex para SEGMENTAR a string: vírgula, espaços, seguida por (lookahead)
        // letra maiúscula, qualquer coisa, espaço, (categoria) no final da parte.
        const segmentRegex = /,\s*(?=[A-ZÀ-ÖØ-Ý].*?\s+\([^)]+\)$)/;
        ...
        for (const rawResponse of rawResponsesArray) {
            ...
            // *** SEGMENTA a string usando a regex de lookahead ***
            const parts = responseString.split(segmentRegex).map(p => p.trim()).filter(p => p !== '');
            const totalParts = parts.length;

            if (totalParts === 0) continue;

            let partsMatchingFormat = 0; // Quantas partes DESTA resposta batem com o formato

            // *** Aplica a regex de extração a CADA parte SEGMENTADA ***
            for (const part of parts) {
                const match = part.match(specialPartRegex);
                if (match) {
                    partsMatchingFormat++; // Parte válida!
                    const responseText = match[1].trim();
                    const category = match[2].trim();

                    // Agrega contagem e categoria SEMPRE que uma parte bater
                    if (!aggregatedCounts[category]) aggregatedCounts[category] = {};
                    if (!aggregatedCounts[category][responseText]) aggregatedCounts[category][responseText] = 0;
                    aggregatedCounts[category][responseText]++;
                    foundCategories.add(category);
                } else {
                     // Log opcional para partes que não bateram APÓS a segmentação
                     console.warn(`   -> Parte segmentada não bateu com regex de extração: "${part}" (original: "${responseString.substring(0, 50)}...")`);
                }
            }

            // Verifica se a resposta atual atende ao critério de % das partes VÁLIDAS
            // (ou seja, das partes que deveriam estar no formato Resposta(Categoria))
            if (partsMatchingFormat > 0 && (partsMatchingFormat / totalParts) >= MIN_PART_PERCENTAGE) {
                responsesMeetingPartCriterion++;
            }
            // Se partsMatchingFormat for 0, a resposta inteira não tinha o formato esperado.
            else if (partsMatchingFormat === 0 && totalParts > 0) {
                 console.warn(`   -> Nenhuma parte válida encontrada na resposta: "${responseString.substring(0,50)}..."`);
            }
        }

        return {
            totalValidResponses,
            responsesMeetingPartCriterion, // Quantas RESPOSTAS passaram no critério 50% das partes
            aggregatedCounts,              // Contagens de TODAS as PARTES que bateram com o formato
            foundCategories                // Categorias de TODAS as PARTES que bateram com o formato
        };
    }; // Fim analyzeSpecialTeiaFormat
    ...
    const gerarVisualizacaoData = (consultaAtualizada, tabelasDados) => {
        ...
        for (let i = 1; i < consultaAtualizada.length; i++) {
            ...
            const currentItemData = { /* ... (estrutura base) ... */
                id: identificador, type: tipoGrafico, title: getVal(indiceTitulo) || `Indicador. ${identificador}`, dimensionNumber: numeroDimensao, dimensionName: getVal(indiceDimensao) || `Dim. ${numeroDimensao || '?'}`, indicatorNumber: numeroIndicador, indicatorName: getVal(indiceIndicador) || `Ind. ${numeroIndicador || '?'}`, dfPerg: { columns: ['Grupo', 'Pergunta'], data: [] }, groupSizes: {}, subGroupToBaseGroup: {}
            };
